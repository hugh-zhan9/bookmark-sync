...
----------------------------------------
## [2026-03-10 16:26] [Docs]
- **Change**: 新增数据源切换 UI 设计文档
- **Risk Analysis**: 设计文档缺失可能导致实现偏离需求
- **Risk Level**: S3（低级: 轻微行为偏差或日志/可观测性影响）
- **Changed Files**:
- `docs/plans/2026-03-10-data-source-toggle-ui-design.md`
----------------------------------------
## [2026-03-10 16:37] [Docs]
- **Change**: 补充数据源切换 UI 实现计划
- **Risk Analysis**: 计划文档变更不完整可能导致实现偏差
- **Risk Level**: S3（低级: 轻微行为偏差或日志/可观测性影响）
- **Changed Files**:
...
- `src/App.test.tsx`
----------------------------------------
## [2026-03-10 17:34] [Bugfix]
- **Change**: 修正数据源切换调用参数并补充类型约束
- **Risk Analysis**: 调用参数结构变更若与后端不一致可能导致设置失败
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- `src/App.tsx`
- `src/App.test.tsx`
----------------------------------------
## [2026-03-10 17:49] [Bugfix]
- **Change**: 设置 Vite base 为相对路径以避免打包后资源加载失败
- **Risk Analysis**: 若 base 设置不兼容开发环境可能影响本地调试或资源路径
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- `vite.config.ts`
----------------------------------------
...
- `src/main.tsx`
----------------------------------------
## [2026-03-10 18:31] [Bugfix]
- **Change**: 修复同步命令中 router 锁重入导致的死锁
- **Risk Analysis**: 同步命令改为一次获取连接快照，可能影响异常分支下的连接获取与错误提示，但逻辑保持一致
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- `src-tauri/src/lib.rs`
----------------------------------------
